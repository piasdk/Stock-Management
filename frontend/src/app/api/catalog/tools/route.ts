...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(`${API_BASE_URL}/catalog/tools`);
    if (companyId) {
      ...
      return NextResponse.json(
        {
          error:
            data?.error ||
            `Failed to fetch tools (status ${response.status})`,
        },
    ...
    return NextResponse.json(
      { error: "Unexpected error fetching tools" },
      { status: 500 },
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/catalog/tools`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        authorization: authHeader,
      ...
      return NextResponse.json(
        {
          error:
            data?.error ||
            `Failed to create tool (status ${response.status})`,
        },
        { status: response.status || 500 },
      );
    }
    ...
    return NextResponse.json(
      { error: "Unexpected error creating tool" },
      { status: 500 },
    );
  }
}
