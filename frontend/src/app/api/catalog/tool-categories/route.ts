...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(`${API_BASE_URL}/catalog/tool-categories`);
    if (companyId) {
      ...
      return NextResponse.json(
        {
          error:
            data?.error ||
            `Failed to fetch tool categories (status ${response.status})`,
        },
    ...
    return NextResponse.json(
      { error: "Unexpected error fetching tool categories" },
      { status: 500 },
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/catalog/tool-categories`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        authorization: authHeader,
      ...
      return NextResponse.json(
        {
          error:
            data?.error ||
            `Failed to create tool category (status ${response.status})`,
        },
        { status: response.status || 500 },
      );
    }
    ...
    console.error("Tool category creation error:", error);
    return NextResponse.json(
      { error: "Unexpected error creating tool category" },
      { status: 500 },
    );
  }
}
