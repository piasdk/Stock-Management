...
export async function GET(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          error:
            data?.error ||
            `Failed to fetch sales (status ${response.status})`,
        },
    ...
    return NextResponse.json(
      { error: "Unexpected error fetching sales" },
      { status: 500 },
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          error:
            data?.error ||
            `Failed to create sales order (status ${response.status})`,
        },
        { status: response.status || 500 },
      );
    }
    ...
    return NextResponse.json(
      { error: "Unexpected error creating sales order" },
      { status: 500 },
    );
  }
}
