...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(`${API_BASE_URL}/production/bom`);
    if (companyId) {
      url.searchParams.set("company_id", companyId);
    }

    console.log(`[BOM API] Fetching from: ${url.toString()}`);
    ...
    if (!response.ok) {
      const errorText = data?.error || `Failed to fetch BOM items (status ${response.status})`;
      console.error("Backend error fetching BOM items:", errorText);
    ...
    console.log(`[BOM API] Backend returned ${bomArray.length} BOM items`);
    if (bomArray.length > 0) {
      console.log(`[BOM API] Sample data:`, bomArray[0]);
    } else {
      console.log(`[BOM API] No BOM items found in database`);
    }
