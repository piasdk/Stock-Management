...
const OperationsManagerDashboard = () => {
  ...
  const navigation = [
    {
      title: 'Core',
      items: [
        { id: 'dashboard', label: 'Operations Command Center', icon: BarChart3 },
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case 'dashboard':
      default:
        // Mock data for Operations Command Center
  ...
  if (!isAuthorized) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <ErrorMessage error="You are not authorized to access the Operations Dashboard." />
      </div>
    );
  }
