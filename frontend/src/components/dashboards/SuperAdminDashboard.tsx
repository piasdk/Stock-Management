...
const SuperAdminDashboard = () => {
  ...
  useEffect(() => {
    if (!mounted || !user) return;
    if (!shouldSeeDashboard(user, "/dashboard/super-admin")) return;
  ...
  const fetchDashboardData = async () => {
    ...
    setError(null);
      ...
      // Try to fetch accounting overview
      const overviewRes = await api.get<any>("/accounting/dashboard-overview");
      ...
      // Try to load company name and currency
      if (user.company_id) {
        ...
        if (!companyRes.error && companyRes.data) {
          ...
          // Set currency from company if not already set from overview
          if (companyRes.data.currency && !currency) {
            setCurrency(companyRes.data.currency.toUpperCase() || "USD");
          }
        }
      }
    } catch (err) {
  ...
  const formatCurrency = (value: number) => {
    // Use the currency from API response, default to USD
    const currencyCode = currency || "USD";
    
    // Map currency codes to locale if needed
    // RWF (Rwandan Franc) doesn't have a standard locale, so we'll use 'en-RW' or format manually
    let locale = 'en-US';
    if (currencyCode === 'RWF') {
      // Format RWF manually since Intl.NumberFormat might not support it well
      ...
      <div className="space-y-4 sm:space-y-6 bg-slate-50 p-3 sm:p-4 md:p-6">
        ...
        <header className="rounded-2xl bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <p className="text-sm text-gray-500 mt-1">Financial overview and system-wide management</p>
              </div>
              ...
              <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-semibold text-gray-900">All Active</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
